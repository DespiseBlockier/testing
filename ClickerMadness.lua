...
function hitboss(Bosstype)
    spawn(function()
        while attackboss == true do    
 
game:GetService("ReplicatedStorage"):WaitForChild("Aero"):WaitForChild("AeroRemoteServices"):WaitForChild("CursorCannonService"):WaitForChild("FireBoss"):FireServer(Bosstype)
task.wait(0.1)
        end
    end)
end
...
local z = w:CreateFolder("Boss Farming")
...
credits:Button("blockiergrub#3108 (500986352045654016)",function()
    print("blockiergrub#3108", "500986352045654016")
end)
...
local Bosstyp
z:Dropdown("boss type",{"Zombie"},true,function(value) --true/false, replaces the current title "Dropdown" with the option that t
    Bosstyp = value
    print(value)
end)

z:Toggle("attack boss",function(bool)
    getgenv().attackboss= bool
    print('attack boss is ', bool)
    if bool and Bosstyp then
        hitboss(Bosstyp)
    end
end)
